...
def spiciest_foods(spicy_foods)
  spicy_foods.select { |food| food[:heat_level] > 5 ? food : nil}
end
...
def get_spicy_food_by_cuisine(spicy_foods, cuisine)
  spicy_foods.select {|food| food[:cuisine] == cuisine}[0]
end
...
def print_spiciest_foods(spicy_foods)
  spicy_foods.each do |food| 
    food[:heat_level] > 5 ? (puts "#{food[:name]} (#{food[:cuisine]}) | Heat Level: #{"🌶" * food[:heat_level]}"): nil
  end
end
